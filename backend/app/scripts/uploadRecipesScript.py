...
def upload_recipes():
    ...
    if 'Image_Name' in df.columns:
        ...
        df['image_object'] = df['Image_Name'].apply(lambda x: load_image(image_folder_path, x))
    ...
    for row in df.itertuples(name='Pandas'):
        try:
            ...
            if image_object:
                ...
                imageS3Filename = "recipes/recipe-" + str(new_recipe_id) + "-" + str(int(random.random() * 10000)) + '.jpg'
            ...
            cur.execute(
                ...
                """,
                (new_recipe_id, userID, "Cheeseburgers", datetime.now(), title, "original caption made by " + str(userID), imageS3Filename)
            )
