...
class BasicRecipeInfo:
    def __init__(self, recipeID, postedByUserID, fullRecipeString, createdDate, title, caption, imageS3Filename, firstName, lastName, profilePicS3Filename, numLikes, userLiked, userRated):
    ...
    def get(recipeID: int):
        ...
        if rows:
            ...
            recipe_info = BasicRecipeInfo(*(rows[0]), numLikes = num_likes, userLiked = user_liked_recipe, userRated = user_rated_recipe)
            return recipe_info
        else:
            return None